...
jfile = "dev-v1.1.json" #sys.argv[1] if len(sys.argv) > 1 else "train-v1.1.json"
...
charnum = 0
linenum = 1
lines = list(csv.reader(open('meta-dev.csv', 'r')))
for i in tqdm.trange(len(js['data'])):
    for j in range(len(js['data'][i]['paragraphs'])):
        ...
        for k, utterance in enumerate(awesome_tokenize(utterances)):
            ...
            assert ID == lines[linenum][0]
            if k == 0:
                SPK = lines[linenum][1]
                shutil.copy2('{}/{}.mp3'.format('dev_audios', ID), '{}/{}.mp3'.format('audios', ID))
                shutil.copy2('{}/{}.lab'.format('dev_audios', ID), '{}/{}.lab'.format('audios', ID))
                csvfile.writerow(lines[linenum])
            else:
                TEXT = utterance
